...
- (void)addImageView{
    ...
    UIImageView * imageView = [[UIImageView alloc]initImageViewWidth:self.bounds.size.width X:self.portraintX Y:self.portraintY circleColor:[UIColor clearColor] borderWidth:0 isRound:NO];
    UITapGestureRecognizer *portraitTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(editPortrait)];
    [imageView addGestureRecognizer:portraitTap];
    self.tapGesture = portraitTap;
...
- (id)initWithWithWidth:(CGFloat)imageW portraitX:(CGFloat)X portraitY:(CGFloat)Y borderWidth:(CGFloat)borderW circleColor:(UIColor *)borderColor isRound:(BOOL)round andViewController:(UIViewController *)controller
{
    ...
    if (self = [super init]) {
        ...
        tempFrame.size.width = imageW;
        tempFrame.size.height = imageW;
        ...
        self.frame = tempFrame;
          ...
          [self addImageView];
//        UIImageView * imageView = [[UIImageView alloc]initImageViewWidth:imageW X:0 Y:0 circleColor:borderColor borderWidth:borderW isRound:round];
//        UITapGestureRecognizer *portraitTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(editPortrait)];
//        [imageView addGestureRecognizer:portraitTap];
//        
//        self.tapGesture = portraitTap;
...
- (void)editPortrait {
      ...
      [self endEditing:YES];
   UIViewController * vc = [AppCurrentControllerTool getCurrentController];
...
- (void)imageCropper:(VPImageCropperViewController *)cropperViewController didFinished:(UIImage *)editedImage {
    ...
    if (self.uploadPictBlock) {
        self.uploadPictBlock(editedImage);
    }else if ([self.portaitDelegate respondsToSelector:@selector(uploadPict:)])
    {
        [self.portaitDelegate uploadPict:editedImage];
    }
//    NSLog(@"裁剪--%@，未裁剪===%@",editedImage,self.origalImage);
