    ...
    [self enumerateAttribute:NSAttachmentAttributeName inRange:NSMakeRange(0, self.length)
                  ...
                  usingBlock:^(id value, NSRange range, BOOL *stop) {
                      ...
                      if (value && [value isKindOfClass:[ImageTextAttachment class]]) {
                          ...
                          ImageTextAttachment * imageAttachment = value;
                          
                          if (imageAttachment.imageUrlKey == nil && ![imageAttachment.imageUrlKey isEqualToString:url]){
                              imageAttachment.imageUrlKey = url;
                              imageAttachment.image = image;
                              imageAttachment.imageSize = [self getMaxSize:image];
...
- (AriclePushlishParam *)getPlainAndImageArray{
    ...
    AriclePushlishParam  * ariclePushlishParam = [[AriclePushlishParam alloc]init];
    NSMutableArray * array = [NSMutableArray array];
    [self enumerateAttribute:NSAttachmentAttributeName inRange:NSMakeRange(0, self.length)
                     options:0
                  usingBlock:^(id value, NSRange range, BOOL *stop) {
                      
                     // NSLog(@"value===%@,class===%@,range==%@",value,[value class],NSStringFromRange(range));
                      
                      // 图片
                      if (value && [value isKindOfClass:[ImageTextAttachment class]]) {
                          
                       
                          AriclePushlishContentParam * ariclePushlishContentParam  = [[AriclePushlishContentParam alloc]init];
                          ariclePushlishContentParam.type = kContentTypeImg;
                          ariclePushlishContentParam.content = kImgtype;
                          [array addObject:ariclePushlishContentParam];
                          //字符串
                      }else
                      {
                      
                          AriclePushlishContentParam * ariclePushlishContentParam  = [[AriclePushlishContentParam alloc]init];
                          ariclePushlishContentParam.type = kContentTypeText;
                          ariclePushlishContentParam.content = [self.string substringWithRange:range];
                          [array addObject:ariclePushlishContentParam];
                      }
    ...
    ariclePushlishParam.content = array;
    return ariclePushlishParam;
